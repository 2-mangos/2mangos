...
interface DashboardProps {
  data: any // (Mantido simplificado para o exemplo, use os seus tipos originais aqui)
...
export default function DashboardClient({ data, userProfile, selectedMonth, selectedYear }: DashboardProps) {
  ...
  const activeBudgets = useMemo(() => {
    return (data.accountsList || []).filter((acc: any) => acc.monthly_budget > 0).map((acc: any) => {
  ...
  return (
    <div className="w-full flex-1 space-y-6 sm:space-y-8 animate-in fade-in duration-500 pb-10">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 w-full">
        ...
        {/* Gráfico Principal (6 colunas = 50% no Desktop) */}
        ...
        {/* Perfil de Despesas (3 colunas = 25% no Desktop) */}
                    ...
                    <div className="w-full h-[140px] relative">
                        <ResponsiveContainer width="100%" height="100%">
                            <PieChart>
                                ...
                                <Tooltip contentStyle={{ borderRadius: '8px', background: '#18181b', border: '1px solid #27272a', color: '#fff', fontSize: '12px' }} formatter={(val: number, name: string) => [formatCurrency(val), name]} />
                            </PieChart>
                        </ResponsiveContainer>
        ...
        {/* Origem de Receitas (3 colunas = 25% no Desktop) */}
        <div className="lg:col-span-6 xl:col-span-3 card p-5 sm:p-6 flex flex-col h-[350px]">
      ...
      {/* LINHA 3: EVOLUÇÃO + CATEGORIAS + METAS */}
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 w-full">
        
        {/* Evolução de Contas (6 Colunas) */}
        <div className="lg:col-span-12 xl:col-span-6 card p-5 sm:p-6 flex flex-col min-h-[350px]">
          <div className="mb-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        ...
        <div className="lg:col-span-6 xl:col-span-3 card p-5 sm:p-6 flex flex-col relative overflow-hidden h-[350px]">
           ...
           ) : ( <div className="flex-1 flex flex-col items-center justify-center text-zinc-600 gap-3"><Target size={24} className="opacity-30" /><p className="text-[10px] text-center">Sem limites configurados.</p></div> )}
        </div>
      </div>

      {/* LINHA 4: CARTÕES E OPERAÇÕES */}
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 w-full mt-2">
          
          {/* Cartão Virtual (4 Colunas) */}
          <div className="lg:col-span-12 xl:col-span-4 card rounded-[2rem] p-6 sm:p-8 flex flex-col justify-center items-center relative overflow-hidden bg-zinc-900/50 shadow-xl border-white/5 min-h-[400px]">
             {userProfile.plan === 'free' && <div className="absolute inset-0 bg-zinc-950/90 backdrop-blur-sm z-30 flex flex-col items-center justify-center text-center p-6"><Lock className="text-amber-400 mb-2" size={24}/><h3 className="font-bold text-white mb-4">Painel Fatura Pro</h3><button onClick={() => setShowUpgradeModal(true)} className="bg-indigo-600 text-white px-6 py-2 rounded-full text-sm font-bold">Desbloquear</button></div>}
          ...
          {/* Últimas Compras (5 Colunas) */}
          <div className="lg:col-span-7 xl:col-span-5 card p-6 sm:p-8 flex flex-col min-h-[400px]">
             <div className="flex justify-between items-center mb-6 shrink-0 border-b border-white/5 pb-4">
                ...
                <button onClick={() => router.push('/cards')} className="flex items-center gap-1.5 text-xs font-bold text-indigo-400 bg-indigo-400/10 px-3 py-1.5 rounded-lg">Ver Cartões <ArrowRight size={14}/></button>
             </div>
             <div className="flex-1 overflow-y-auto pr-2 space-y-1 custom-scrollbar">
                {recentCcTransactions.length > 0 ? recentCcTransactions.slice(0, 6).map((t: any, i: number) => (
                    <div key={i} className="flex items-center justify-between p-3 rounded-xl hover:bg-zinc-900/50 border border-transparent">
          ...
          {/* Operações (3 Colunas) */}
          <div className="lg:col-span-5 xl:col-span-3 card p-5 flex flex-col h-[400px] w-full">
