...
export default function CardsClient({ initialAccounts, userPlan }: CardsClientProps) {
  ...
  return (
    /* ALINHAMENTO BASEADO EM /INCOMES E /EXPENSES: Container principal idêntico */
    <div className="min-h-screen p-8 pb-32">
      <div className="mx-auto max-w-7xl space-y-6 animate-in fade-in duration-500">
        
        {/* HEADER PADRONIZADO (Com borda inferior e alinhamento md:items-end) */}
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 pb-4 border-b border-white/5">
          ...
          <div className="flex items-center gap-3">
            <div className="flex items-center bg-zinc-900/50 border border-white/10 rounded-lg p-1">
               <select value={selectedMonth} onChange={(e) => handleFilterChange(parseInt(e.target.value), selectedYear)} className="bg-transparent text-zinc-300 text-sm font-medium py-1.5 pl-3 pr-2 cursor-pointer outline-none [&>option]:bg-zinc-900">
                 {monthNames.map((m, i) => (<option key={i} value={i}>{m}</option>))}
               </select>
               <div className="relative border-l border-white/5">
                  <select value={selectedYear} onChange={(e) => handleFilterChange(selectedMonth, parseInt(e.target.value))} className="bg-transparent text-zinc-300 text-sm font-medium py-1.5 pl-2 pr-6 cursor-pointer outline-none [&>option]:bg-zinc-900">
        ...
        {/* SELETOR DE CARTÕES (Ajustado espaçamento superior para combinar com a nova estrutura do header) */}
            ...
            <div className="lg:col-span-3 space-y-6">
              ...
              <div className="card p-6 border-white/5">
                ...
                <div className="space-y-3">
                  ...
                  <InsightBar text={`Atenção: Seu uso de limite está em ${percentualUso}%. ${Number(percentualUso) > 75 ? 'Considere reduzir gastos variáveis.' : 'Mantenha o controle para avoid juros.'}`} />
                </div>
              </div>
            </div>
