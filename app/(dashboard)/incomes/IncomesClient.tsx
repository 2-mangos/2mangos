                                    ...
                                    return (
                                        <tr key={inc.id} className={`transition-all group ${isSelected ? 'bg-emerald-500/10' : 'hover:bg-white/5'}`}>
                                            <td className="px-6 py-3.5">
                                                <button onClick={() => handleSelectOne(inc.id)} className="text-zinc-500 hover:text-white transition-colors">
                                            ...
                                            <td className="px-6 py-3.5 text-right text-xs font-medium relative">
                                                {editingId === inc.id ? (
                                                    <div className="flex justify-end gap-1.5">
                                                        <button onClick={() => handleSaveEdit(inc.id)} className="text-white bg-emerald-600 hover:bg-emerald-500 p-1.5 rounded-md transition-colors"><Save size={14}/></button>
                                                    ...
                                                    <div className="flex justify-end opacity-0 group-hover:opacity-100 transition-opacity">
                                                        <button onClick={(e)=>{e.stopPropagation(); handleToggleMenu(inc.id)}} className="text-zinc-400 hover:text-white p-1.5 hover:bg-white/10 rounded-md transition-colors"><MoreVertical size={16}/></button>
                                                        {openMenuId === inc.id && (
                                                            <div ref={menuRef} className="absolute right-10 top-2 z-50 w-32 bg-zinc-800 shadow-xl rounded-xl border border-white/10 overflow-hidden py-1 animate-in fade-in zoom-in-95">
                                                                <button onClick={()=>handleStartEdit(inc)} className="w-full px-4 py-2.5 hover:bg-white/5 text-zinc-200 text-xs font-medium flex items-center gap-2 transition-colors"><Edit2 size={12}/> Editar</button>
                                                                <div className="h-px bg-white/5 mx-2 my-0.5"></div>
                                                                <button onClick={()=>handleDelete(inc.id)} className="w-full px-4 py-2.5 hover:bg-rose-500/10 text-rose-400 text-xs font-medium flex items-center gap-2 transition-colors"><Trash2 size={12}/> Deletar</button>
                                                            </div>
                                                        )}
                                                    </div>
                                                )}
